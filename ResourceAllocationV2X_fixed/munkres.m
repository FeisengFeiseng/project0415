...
% 线性分配问题的Munkres（匈牙利）算法
% [ASSIGN,COST] = munkres(COSTMAT) returns the optimal column
% indices,（最佳列索引）
% ASSIGN assigned to each row and the minimum COST based on the assignment
% problem represented by the COSTMAT, where the (i,j)th element represents the cost to assign the jth
% job to the ith worker.
% 根据COSTMAT表示的分配问题分配给每一行和最小成本，其中第（i，j）个元素表示将第j个
% 作业分配给第i个工人的成本。
...
% assigned tasks.
% 部分赋值：如果完全赋值不可行，则此代码可以识别部分赋值。对于部分分配，返回的分配
% 向量中有一些零元素，表示未分配的任务。返回的成本仅包含部分分配任务的成本。
% This is vectorized implementation of the algorithm. It is the fastest
% among all Matlab implementations of the algorithm.
% 这是算法的矢量化实现。它是所有Matlab算法实现中速度最快的。
% Examples
% Example 1: a 5 x 5 example
%{
[assignment,cost] = munkres(magic(5));
magic() 用于产生魔方矩阵，它的每行、列以及对角线的数之和相等。
该和的值为1+2+3+.....+n^2的和再除以n，n必须为大于或等于3的整数。
